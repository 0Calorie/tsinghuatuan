...
$('#activity-photo').change(function () {
 /*   window.URL = window.URL || window.webkitURL;
    var imgFile = document.getElementById('fileToUpload');
    if (window.URL) {
        //File API
        $('#activity-photo').attr("src", window.URL.createObjectURL(imgFile.files[0])); //创建一个object URL，并不是你的本地路径

        $('#activity-photo').onload = function (e) {
